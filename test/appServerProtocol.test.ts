...
describe("app-server protocol projection", () => {
	...
	it("maps app-server goal statuses onto local goal statuses", () => {
		...
		expect(normalizeGoal({ status: "usageLimited" }).status).toBe(
			"usage_limited",
		);
		expect(normalizeGoal({ status: "budgetLimited" }).status).toBe(
			"budget_limited",
		);
	...
	it("rejects unknown app-server runtime statuses", () => {
		...
		expect(() => normalizeTurnStatus("running")).toThrow(
			/Unknown app-server turn status/,
		);
	});
	...
	it("classifies missing rollout errors as thread-not-found runtime drift", () => {
		const error = requestError(
			{
				message:
					"no rollout found for thread id 019ee0dd-6f13-7043-995f-d88646e16316",
			},
			{
	...
	it("builds fixed yolo approval responses at the app-server boundary", () => {
		expect(
			yoloApprovalResponse("item/commandExecution/requestApproval", {}),
		).toEqual({
			decision: "accept",
		});
		expect(
