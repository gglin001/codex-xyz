...
import type { ControlThread, DashboardState, TerminalSnapshot, ThreadPage } from "../src/server/domain.js";
...
describe("Next API routes", () => {
  it("serves dashboard state and can create a local session", async () => {
    ...
    const created = await json<{ thread: { id: string; cwd: string } }>("/api/threads", {
      method: "POST",
      body: JSON.stringify({
        cwd: tempDir,
  ...
  it("creates a goal session from a direct thread request", async () => {
    ...
    await waitFor(async () => {
      const detail = await json<{ turns: Array<{ prompt: string; status: string }> }>(
  ...
  it("controls a running session through core API routes", async () => {
    ...
    await waitFor(() => service.getThreadDetail(created.thread.id).activeTurnId !== null, "running turn");

    const repeatedTurn = await json<{ threadId: string }>(`/api/threads/${created.thread.id}/turns`, {
      method: "POST",
      body: JSON.stringify({
