...
export class ControlService {
  ...
  async clearGoal(threadId: string) {
    ...
    return this.projection.updateGoal(threadId, null, null, { clearedStatus: "cleared" });
  }
  ...
  private async steerActiveTurn(thread: ControlThread, prompt: string) {
    ...
    this.projection.publish("turn.steered", thread.id, activeTurnId, { prompt });
  }
  ...
  private async resumeRuntimeThread(thread: ControlThread) {
    try {
      ...
      this.projection.publish("thread.resumed", thread.id, null, { thread: this.store.getThread(thread.id) });
  ...
  private async createContinuationThread(source: ControlThread, continuation: RuntimeContinuation) {
    ...
    this.projection.publish("thread.continued", thread.id, null, { thread, sourceThreadId: source.id });
    return thread;
  }
