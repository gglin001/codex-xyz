...
export function isTranscriptProcessItem(item: ThreadItem, directAgentIds: Set<string>) {
  ...
  return item.type === "command" || item.type === "plan" || item.type === "system" || processSourceTypes.has(sourceType ?? "");
}
